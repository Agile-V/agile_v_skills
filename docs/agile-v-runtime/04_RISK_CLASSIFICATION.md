...
4. `L3`/`L4` require independent findings and a human decision; AI output is never independent assurance. A fresh context alone is at most `I1`; a separately instantiated, role-separated verifier agent with protected, independent inputs (Red Team Protocol) may reach `I2`; AI separation alone — same or different agent, with or without role separation — never satisfies an `I3`/`I4` requirement on its own (see `08_INDEPENDENCE_CLASSES.md`).
