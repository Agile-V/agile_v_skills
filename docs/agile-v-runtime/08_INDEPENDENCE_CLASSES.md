...
4. **"Independent verification" (Red Team Protocol) and "independent assurance" (regulatory/organizational) are different claims.** `docs/agile-v-runtime/04_RISK_CLASSIFICATION.md` requires I2 role-separated verification from `L1` upward and reserves I3/I4 for `L3`/`L4` as stated there; a second AI agent alone never satisfies an I4 requirement.
