...
|--------|-------|--------|-------------------|---------------------------|
| 4.1.6 | Validation of QMS software | **PARTIAL (SCHEMA-BACKED, draft)** | `agile-v-gxp-qualification` provides draft assurance-toolchain qualification contracts (plan, system description, baseline, IQ/OQ/PQ protocols/executions). DQ/IQ/OQ/PQ are evidence stages, not agent names. | Toolchain qualification (Agile V skills + runtime + model config) is a scope SEPARATE from the target device software. **User must:** execute IQ/OQ/PQ against approved protocols and acceptance criteria; a Markdown/schema contract is not executed qualification. |
...
| 7.3.6 | Design validation | **PARTIAL** | Human Gate 2 serves as acceptance. `validation-agent` provides distinct intended-use validation (planning, protocol, report, deviations), separate from `red-team-verifier` verification. | Verification and validation are now distinguished (no longer conflated), but intended-use validation execution in representative conditions remains an organizational activity. **User must:** Conduct separate validation in intended-use conditions (simulated or clinical) via `validation-agent`; record representative users, environment, and data. |
