...
4. Link AI inventory to task ID, REQ-IDs, ART-IDs, test results, evidence bundle, and release package.
...
3. Capture observed AI context where available (runtime logs, k8s-aibom, CI hooks, session metadata).
...
| L1 | L0 + provider/runtime + loaded Agile-V skills + repository commit + tool list |
| L2 | L1 + model version/deployment + agent runtime + RAG/context sources + evidence links |
| L3 | L2 + BOM diff + no unresolved model/runtime/tool fields + independent verifier review |
